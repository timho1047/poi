...
if __name__ == "__main__":
    config = LLMConfig(run_name="llama3-nyc-multi-gpu-5", num_epochs=2, batch_size=2, gradient_accumulation_steps=1)

    DATASET_DIR = settings.DATASETS_DIR / "NYC"

    train_dataset = load_llm_dataset(DATASET_DIR / "LLM Dataset" / "train_codebook.json", max_examples=36)
    eval_dataset = load_llm_dataset(DATASET_DIR / "LLM Dataset" / "test_codebook.json", max_examples=8)
