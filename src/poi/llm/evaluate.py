...
def evaluate_model(config: LLMConfig, ds_dir: Path, from_hub: bool = True):
    ...
    for file in ds_dir.glob("*.json"):
        if file.name.startswith("train"):
            ...
            test_ds_path = file
    if not all([train_ds_path, val_ds_path, test_all_ds_path, test_ds_path]):
        raise ValueError(f"No train, val, test or test_all dataset found in {ds_dir}")

    train_ds = load_prompt_completion_llm_dataset(train_ds_path)
    val_ds = load_prompt_completion_llm_dataset(val_ds_path)
    test_all_ds = load_prompt_completion_llm_dataset(test_all_ds_path)
    ...
    val_acc = top_one_accuracy(config, model, val_ds, is_sid)
    test_all_acc = top_one_accuracy(config, model, test_all_ds, is_sid)
    test_acc = top_one_accuracy(config, model, test_ds, is_sid)

    return {
        "train_acc": train_acc,
        "val_acc": val_acc,
        "test_all_acc": test_all_acc,
        "test_acc": test_acc,
    }
...
def evaluate_all_and_save(config_ds_dir_pairs: list[tuple[LLMConfig, Path]], save_path: Path, from_hub: bool = True):
    ...
    for config, ds_dir in pbar:
        ...
        with save_path.open("a") as f:
            f.write(
                f"{config.run_name},{metrics['train_acc']:.4f},{metrics['val_acc']:.4f},{metrics['test_all_acc']:.4f},{metrics['test_acc']:.4f}\n"
            )
        pbar.write(f"Evaluated {config.run_name} with metrics: {metrics}")
        pbar.update(1)
